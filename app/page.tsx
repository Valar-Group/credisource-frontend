  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A1628] via-[#152542] to-[#1a1535]">
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-3 gap-8">
              <StatCard number="8M" label="deepfakes by the end of 2025" />
              <StatCard number="$40b" label="AI fraud by 2027" />
              <StatCard number="90%" label="of online content AI-generated by 2026" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-[#E91E8C]/10 to-[#7B2D8E]/10 py-20">
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-4xl font-bold text-white mb-4">Display the badge,</h2>
            <p className="text-4xl font-bold text-[#E91E8C] mb-12">harness trust.</p>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 gap-4">
        <TypeButton icon={<ImageIcon />} label="Image" selected={selectedType === 'image'} onClick={() => setSelectedType('image')} />
        <TypeButton icon={<VideoIcon />} label="Video" selected={selectedType === 'video'} onClick={() => setSelectedType('video')} />
        <TypeButton icon={<TextIcon />} label="Text" selected={selectedType === 'text'} onClick={() => setSelectedType('text')} />
        <TypeButton icon={<NewsIcon />} label="News" selected={selectedType === 'news'} onClick={() => setSelectedType('news')} />
      </div>

      {selectedType && (
        <div
          className={`border-2 border-dashed rounded-xl p-12 transition-all ${isDragging ? 'border-[#E91E8C] bg-[#E91E8C]/10' : 'border-white/20 hover:border-white/40'}`}
        ...
        >
          <div className="text-center space-y-4">
            ...
            <div className="text-white text-lg">Paste text, enter a URL or drop an {selectedType} to verify</div>
            
            <div className="flex gap-3 max-w-2xl mx-auto">
              <input type="text" placeholder={`Enter ${selectedType} URL...`} className="flex-1 bg-white/10 border border-white/20 rounded-lg px-6 py-4 text-white placeholder-white/50 focus:outline-none focus:border-[#E91E8C] transition-colors" />
              <button className="bg-[#E91E8C] hover:bg-[#d11a7d] text-white px-8 py-4 rounded-lg font-semibold transition-colors whitespace-nowrap">Verify</button>
            </div>
      ...
      {!selectedType && <div className="text-center py-12 text-[#B8C5D6]">Select a content type above to get started</div>}
    </div>
  );
}

function TypeButton({ icon, label, selected, onClick }: { icon: React.ReactNode; label: string; selected: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`p-6 rounded-xl transition-all ${selected ? 'bg-[#E91E8C] text-white' : 'bg-white/5 hover:bg-white/10 text-[#B8C5D6]'}`}>
...
function PartnerLogo({ name }: { name: string }) {
  return <div className="text-[#B8C5D6] font-semibold text-sm">{name}</div>;
}
